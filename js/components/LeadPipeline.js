  ...
  return h('div', { className: 'space-y-6' }, [
    h('div', { key: 'pipeline-header', className: 'flex items-center justify-between' }, [
      h('div', null, [
        ...
        h('p', { className: 'text-xs text-slate-400' }, 'Qualify incoming phone volume, attach site inspection logs, and publish leads to the subcontractor queue.')
      ])
    ]),
        ...
        h('div', { className: 'space-y-3 flex-1' }, 
          incomingJobs.length === 0 
            ...
            : incomingJobs.map(job => 
                h('div', { key: job.id, className: 'bg-slate-950 border border-slate-800 p-3.5 rounded-xl space-y-2' }, [
                  ...
                  h('div', { className: 'pt-2 border-t border-slate-800/80 flex items-center justify-between gap-2' }, [
                    ...
                    h('button', {
                      onClick: () => onUpdateStatus(job.id, 'AVAILABLE'),
      ...
      h('div', { className: 'bg-slate-900/60 border border-slate-800 rounded-2xl p-4 flex flex-col' }, [
        ...
        h('div', { className: 'space-y-3 flex-1' }, 
          queueJobs.length === 0 
            ...
            : queueJobs.map(job => 
                h('div', { key: job.id, className: 'bg-slate-950 border border-amber-500/30 p-3.5 rounded-xl space-y-2' }, [
                  h('div', { className: 'flex justify-between items-start gap-2' }, [
                  ...
                  h('div', { className: 'pt-2 border-t border-slate-800 flex justify-between items-center' }, [
                    ...
                    h('span', { className: 'text-[10px] text-amber-400 italic' }, 'Waiting for Sub...')
                  ])
                ])
              )
        )
        ...
        h('div', { className: 'space-y-3 flex-1' }, 
          claimedJobs.length === 0 
            ...
            : claimedJobs.map(job => {
                ...
                return h('div', { key: job.id, className: 'bg-slate-950 border border-emerald-500/30 p-3.5 rounded-xl space-y-2' }, [
                  h('div', { className: 'flex justify-between items-start gap-2' }, [
                    h('h4', { className: 'text-xs font-bold text-white' }, job.title),
                    h('span', { className: 'text-[10px] text-emerald-400 font-bold' }, `$${job.estimatedValue || 0}`)
                  ]),
                  ...
                  h('div', { className: 'text-[10px] text-slate-400 flex justify-between pt-1' }, [
                    h('span', null, `FTD Fee (${job.referralFeePct || 8}%):`),
                    h('span', { className: 'font-bold text-emerald-400' }, `+$${ftdCut}`)
                  ])
                ]);
              })
        )
